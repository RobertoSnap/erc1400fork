...
contract ERC1400Partition is IERC1400Partition, ERC1400Raw {
   ...
   */
    function transferByPartition(
        ...
        uint256 value,
        bytes calldata data
    ) external isValidCertificate(data) returns (bytes32) {
        return
   ...
   */
    ...
    ) external isValidCertificate(operatorData) returns (bytes32) {
        require(
   ...
   * @param value Number of tokens to transfer.
   */
    function _addTokenToPartition(address to, bytes32 partition, uint256 value)
        internal
    {
        if (value != 0) {
   ...
   */
    function transferFromWithData(
        ...
        bytes calldata data,
        bytes calldata operatorData
    ) external isValidCertificate(operatorData) {
