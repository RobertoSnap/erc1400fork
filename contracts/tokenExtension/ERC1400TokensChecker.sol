    ...
    ERC1820Implementer
{
   ...
   */
    function _canTransferByPartition(
        ...
        if (
            !IERC1400Partition(msg.sender).isOperatorForPartition(
                partition,
                operator,
                from
            )
        ...
        if (
            (IERC1400Raw(msg.sender).balanceOf(from) < value) ||
            (IERC1400Partition(msg.sender).balanceOfByPartition(
                partition,
                from
            ) <
                value)
        ...
        if (to == address(0)) return (hex"A6", "", partition); // Transfer Blocked - Receiver not eligible
